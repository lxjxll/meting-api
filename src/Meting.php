        ...
        switch ($this->server) {
            case 'netease':
                $api = array(
                    ...
                    'encode' => 'netease_AESCBC',
                    'format' => 'playlist.trackIds',
                );
                $api = call_user_func_array(array($this, $api['encode']), array($api));
                $this->curl($api['url'], $api['body']);
                $this->data = $this->pickup(json_decode($this->raw, true), $api['format']);
                $this->data = array_map(function ($data) {
                    return array('id' => $data['id']);
                }, $this->data);
                $ids = array_chunk($this->data, 1000)[0];
                $ids = json_encode($ids);
                $api = array(
                    'method' => 'POST',
    ...
    private function netease_url($result)
    {
        ...
        if (isset($data['data'][0]['url'])) {
            ...
            if (isset($data['data'][0]['id'])) {
                ...
                $album = $data[0]['album'];
                $this->site("kuwo");
                $data = $this->search($name . ' ' . explode(" ", $album)[0]);
                $data = json_decode($data, true);
                if (empty($data)) {
                ...
                $url = "https://api.elderly.top/kuwo/song/url";
                $url .= '?' . http_build_query($body);
                $curl = curl_init();
                curl_setopt($curl, CURLOPT_URL, $url);//设置访问的URL
                curl_setopt($curl, CURLOPT_HEADER, false);//设置不需要头信息
                curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
                curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, 0);
                $raw = curl_exec($curl);//执行访问，返回结果
